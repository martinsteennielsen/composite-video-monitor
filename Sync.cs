...
namespace CompositeVideoMonitor {
    ...
    public class PhaseDetector {
        ...
        public PhaseDetector(TimingConstants timing, ISignal reference, ISignal signal) {
            Timing = timing;
            Signal = signal;
            Reference = reference;
        }
        ...
        public void Collect(double time) {
            ...
            bool isSyncSig = Signal.Get(time) < Timing.BlackLevel;
            CurrentPulse(SyncSignalPulses).Collect(time, sync: isSyncSig, min: Timing.MinHSync, max: Timing.MaxHSync);
            CurrentPulse(SyncReferencePulses).Collect(time, sync: isSyncRef, min: Timing.MinHSync, max: Timing.MaxHSync);
        }
        ...
        public bool TryGetPhase(out double phase) {
            ...
            phase = Math.PI * (timeDiff.Value) / Timing.LineTime;
            return true;
        }
