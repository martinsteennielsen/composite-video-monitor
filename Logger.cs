...
namespace CompositeVideoMonitor {
    ...
    public class Logger {
        internal double VPhase { get; set; }
        ...
        public async Task Run(CancellationToken canceller) {
            while (!canceller.IsCancellationRequested) {
                Console.WriteLine($"VPA:{VPhase,5:F2} HPA:{HPhase,5:F2} FPS:{FramesPrSecond,5:F2} SPF:{SimulationsPrFrame,7:F2} DPS:{DotsPrSimulation,6} Dots:{DotCount,7}, Skipped time:{SkippedTime}");
